...
st.set_page_config(page_title="Kharma Store",page_icon="🛍️",layout="wide",initial_sidebar_state="collapsed")
    ...
    if not code:
        ...
        if code:
            with st.spinner('Verificando autenticación, espere por favor...'):
                try:
                    usuario = verificar_o_crear_usuario(code)
                    ...
                    if usuario:
                        ...
                        st.query_params.clear()
                        st.error("❌ Fallo en la verificación del usuario. Intente nuevamente.")
                except Exception as e:
                    st.query_params.clear()
                    st.error(f"❌ Error inesperado durante la autenticación: {e}")
